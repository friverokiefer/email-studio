...
import { type GenerateV2Response, generateEmailsV2, type GenerateV2Payload } from "@/lib/apiEmailV2";
 ...
 * UI Helpers
 * ========================= */
const inputClass = "w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-all";
...
function Collapsible({ title, children, defaultOpen = true }: { title: string; children: React.ReactNode; defaultOpen?: boolean }) {
  ...
  return (
    <div className="rounded-2xl border bg-white shadow-sm overflow-hidden">
      <button type="button" className="w-full flex items-center justify-between gap-2 px-4 py-3 bg-slate-50/50 hover:bg-slate-50 transition-colors" onClick={() => setOpen((v) => !v)}>
 ...
 * ========================= */
export function Email2Sidebar({ onGenerated, currentBatchId }: { onGenerated?: (resp: GenerateV2Response) => void; currentBatchId?: string }) {
  // 1. Hook de Formulario (Estado, Meta, Persistencia)
  const { state, setState, metaLoading, metaError, availableCampaigns, availableClusters, meta } = useSidebarForm();

  // 2. Estado Local de UI (Generación, Historial, Buscador)
  ...
  const [activeBatchId, setActiveBatchId] = useState<string | undefined>(currentBatchId);
  ...
  useEffect(() => { if (currentBatchId) setActiveBatchId(currentBatchId); }, [currentBatchId]);
  ...
  function handlePreGenerate() {
    ...
    if (!state.campaign || !state.cluster) return toast.warning("Selecciona Campaña y Cluster.");
    setConfirmOpen(true);
  }
  ...
  return (
    <div className="space-y-6 font-sans pb-10">
      ...
      <div className="flex items-center justify-between px-1">
        <h3 className="text-lg font-bold text-slate-800 tracking-tight">Parámetros</h3>
        <div className="flex items-center gap-2">
            {isGenerating && <span className="text-[10px] font-bold bg-sky-100 text-sky-700 px-2 py-1 rounded-full animate-pulse">GENERANDO {elapsedLabel}</span>}
            {!isGenerating && activeBatchId && <span className="text-[10px] font-bold bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full">LISTO</span>}
        </div>
      </div>
      ...
      {activeBatchId && (
        <div className="mx-1 px-3 py-2 bg-slate-100 rounded-lg text-xs text-slate-600 flex justify-between items-center">
            ...
            {batchJsonLink && <a href={batchJsonLink} target="_blank" rel="noreferrer" className="text-sky-600 hover:underline">JSON</a>}
        </div>
      )}

      {/* 1. Selección Base */}
      <Collapsible title="📘 Selección de Campaña">
        {metaError && <div className="p-2 bg-rose-50 text-rose-600 text-xs rounded-md">{metaError}</div>}
        
        <div className="space-y-3">
            <div>
            <label className="mb-1 block text-xs font-bold text-slate-700 uppercase">Campaña</label>
            <select
            ...
            <div>
            <label className="mb-1 block text-xs font-bold text-slate-700 uppercase">Cluster / Segmento</label>
            <select
                className={inputClass}
      ...
      <Collapsible title="🧠 Instrucciones IA">
        <div className="space-y-4">
            <div>
                <label className="mb-1 block text-xs font-bold text-slate-700">Feedback Asunto</label>
                <textarea 
                    className={textareaClass} rows={1} placeholder="Ej: Hazlo urgente y corto..." onInput={autoGrow}
                    value={state.feedbackSubject} onChange={e => setState(s => ({...s, feedbackSubject: e.target.value}))}
                />
            </div>
            <div>
                <label className="mb-1 block text-xs font-bold text-slate-700">Feedback Pre-header</label>
                <textarea 
                    className={textareaClass} rows={1} placeholder="Complementa el asunto..." onInput={autoGrow}
                    value={state.feedbackPreheader} onChange={e => setState(s => ({...s, feedbackPreheader: e.target.value}))}
                />
            </div>
            <div>
                <label className="mb-1 block text-xs font-bold text-slate-700">Feedback Cuerpo</label>
                <textarea 
                    className={textareaClass} rows={2} placeholder="Instrucciones para el contenido..." onInput={autoGrow}
                    value={state.feedbackBody} onChange={e => setState(s => ({...s, feedbackBody: e.target.value}))}
                />
            </div>
        </div>
      </Collapsible>
      ...
      <Collapsible title="⚙️ Configuración">
        <div className="grid grid-cols-2 gap-4">
            <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Cant. Sets</label>
                <input type="number" min={1} max={5} className={inputClass} 
                    value={state.setCount} onChange={e => setState(s => ({...s, setCount: parseInt(e.target.value) || 1}))}
                />
            </div>
            <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Cant. Imágenes</label>
                <input type="number" min={1} max={5} className={inputClass} 
                    value={state.imageCount} onChange={e => setState(s => ({...s, imageCount: parseInt(e.target.value) || 1}))}
                />
            </div>
        </div>
      </Collapsible>
                ...
                {filteredHistory.length === 0 ? (
                    ...
                    <ul className="divide-y divide-slate-50">
                        {filteredHistory.map(h => (
                            <li key={h.batchId} className="flex justify-between items-center p-2 hover:bg-slate-50 group cursor-pointer" onClick={() => handleLoadBatch(h.batchId)}>
                                <div>
                                    <div className="font-mono text-[10px] text-slate-600 font-medium group-hover:text-sky-600 transition-colors">{h.batchId}</div>
                                    <div className="text-[9px] text-slate-400">{h.count ?? '-'} items</div>
                                </div>
                                <svg className="w-4 h-4 text-slate-300 group-hover:text-sky-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                            </li>
                        ))}
                    </ul>
                )}
